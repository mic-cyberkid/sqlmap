...
class Dump(object):
    ...
    def pocReport(self):
        if not conf.pocReport:
            return

        poc_dir = conf.pocReport
        if not os.path.isdir(poc_dir):
            os.makedirs(poc_dir)

        logger.info("generating PoC report directory at '%s'" % poc_dir)
        ...
        for injection in kb.injections + kb.falsePositives:
            vuln = {
                ...
                "dbms": injection.dbms,
                "techniques": list(injection.data.keys()),
                "notes": injection.notes
            }
            metadata["vulnerabilities"].append(vuln)

            # Generate exploit_request.sh
            exploit_path = os.path.join(poc_dir, "exploit_request.sh")
            with openFile(exploit_path, "w+") as f:
                f.write("#!/usr/bin/env bash\n")
                f.write("# Generated by sqlmap JSON Enhancements PoC\n")
                f.write("# Target: %s\n" % conf.url)
                f.write("# Parameter: %s\n" % injection.parameter)
                f.write("\n")
                ...
                for stype, sdata in injection.data.items():
                    ...
                    curl_cmd = "curl -i -k -X %s \\\n" % (conf.method or "POST")
                    curl_cmd += "  '%s' \\\n" % conf.url

                    for header, value in conf.httpHeaders:
                        if header.upper() not in ("CONTENT-LENGTH",):
                            curl_cmd += "  -H '%s: %s' \\\n" % (header, value)

                    if injection.place in (PLACE.POST, PLACE.CUSTOM_POST):
                        # Reconstruct the body for this specific injection
                        body = agent.payload(injection.place, injection.parameter, newValue=payload)
                        # Remove sqlmap delimiters for the PoC command
                        body = agent.removePayloadDelimiters(body)
                        f.write("  -d '%s'\n" % body.replace("'", "'\\''"))

                    f.write("\n%s\n\n" % curl_cmd)

        # Write metadata.json
        with openFile(os.path.join(poc_dir, "metadata.json"), "w+") as f:
            import json
            json.dump(metadata, f, indent=4)

        # Write login_request.sh if applicable
        if conf.authUrl:
            login_path = os.path.join(poc_dir, "login_request.sh")
            with openFile(login_path, "w+") as f:
                f.write("#!/usr/bin/env bash\n")
                f.write("curl -i -k -X POST '%s' \\\n" % conf.authUrl)
                f.write("  -H 'Content-Type: application/json' \\\n")
                f.write("  -d '%s'\n" % conf.authData.replace("'", "'\\''"))

        # Write README.md
        readme_path = os.path.join(poc_dir, "README.md")
        with openFile(readme_path, "w+") as f:
            f.write("# SQL Injection PoC Report\n\n")
            f.write("## Target Information\n")
            f.write("* **URL**: %s\n" % conf.url)
            f.write("* **DBMS**: %s\n\n" % Backend.getDbms())
            f.write("## Vulnerabilities\n")
            for vuln in metadata["vulnerabilities"]:
                f.write("### Parameter: `%s` (%s)\n" % (vuln["parameter"], vuln["place"]))
                f.write("* **Techniques**: %s\n" % ", ".join(str(t) for t in vuln["techniques"]))
                f.write("\n")
            f.write("## Reproducing\n")
            f.write("Run `./exploit_request.sh` to reproduce the findings.\n")
